...
class DDPMSampler: # (Scheduler)
    ...
    def set_strength(self, strength=1):
        start_step = self.num_inference_steps - int(self.num_inference_steps * strength) # This means - that if we have strength for example 0.8, we will skip 20% of steps
        self.timesteps = self.timesteps[start_step:]
        self.start_step = start_step

    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor): # Remove noise ()
        """
        model_output: predicted noise by model
        """
        t = timestep
        ...
        # Cmpute the predicted original sample using formula (15) of the DDPM paper
        pred_original_sample = (latents - (beta_prod_t ** 0.5) * model_output) / alpha_prod_t ** 0.5

        # Compute the coeficients for pred_orgiginal_sample and current sample x_t (7)
